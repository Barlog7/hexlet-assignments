...
public class ProductsController {
    ...
    public ProductDTO update(@PathVariable long id, @RequestBody ProductUpdateDTO upDTO ) {
        var product = productRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Product with id " + id + " not found"));
        product = toEntityUFromUpdate(upDTO, product);
        productRepository.save(product);

        var prodactDTO = toDTO(product);
        return prodactDTO;
    }

    private Product toEntityUFromUpdate(ProductUpdateDTO upDTO, Product product) {
        //product = new Product();
        product.setTitle(upDTO.getTitle());
